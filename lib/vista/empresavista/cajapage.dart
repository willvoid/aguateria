...
class _CajaPageState extends State<CajaPage> {
  ...
  void _eliminarCaja(Caja caja) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Confirmar eliminación'),
        content: Text('¿Está seguro de eliminar la caja ${caja.nro_caja} - ${caja.descripcion_caja}?'),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final exito = await _cajaCrud.eliminarCaja(caja.id_caja!);
              Navigator.pop(context);
              ...
              if (exito) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Caja eliminada exitosamente'),
                    backgroundColor: Colors.green,
                  ),
                );
              } else {
                _mostrarError('Error al eliminar la caja');
              }
            },
